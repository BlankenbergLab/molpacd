...
def coords_from_atoms(atoms: Sequence[AtomRecord]) -> np.ndarray:
    if not atoms:
...
def parse_axis(axis: str, coords: np.ndarray) -> np.ndarray:
...
def analyze_openings(
    ...
    options: CapOptions,
) -> Tuple[np.ndarray, np.ndarray, Opening, Opening]:
...
def _coord_tuple(values: Sequence[float]) -> Tuple[float, float, float]:
